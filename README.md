...
聚合 **WYY音乐 / KG音乐 / 哔哩哔哩** 三平台榜单 + **本地歌单** 的音乐播放器，Android + iOS 双端。
...
| 模块 | 说明 |
|---|---|
| 榜单 | WYY（热歌榜/新歌榜）、KG（TOP500/飙升榜）、B站（热门视频），Top50 平铺首页，点歌即播 |
| 搜索 | 各平台歌曲搜索（WYY cloudsearch / KG song_search_v2） |
...
| 歌词 | 内嵌滚动歌词 + 桌面悬浮歌词；无词时点击「暂无歌词」联网自动匹配（WYY→KG）；点击歌词行跳播 |
...
2. **首页能出歌**：平台 Tab（WYY/KG/B站/我的歌单）+ 榜单 Tab，每行排名 + 封面 + 歌名 + 时长；平台不可用时 Tab 挂「维护中」角标（仍可点，进去会重试）；
...
1. **WYY 部分歌曲被拦截**（版权风控），依赖 LX 代理源与 GD 兜底策略，兜不住的歌曲出错误卡片；
2. **KG「飙升榜」rankid `6666` 为二手来源未实测**，空榜时在 `data/kugou/KugouModule.kt` 里把该条 `enabled = false` 或换其他榜单；
3. **WYY「新歌榜」`3779629` 同为二手来源**（同上）；
4. **榜单只有进程内内存缓存**（退出即失）——WYY 直链 20 分钟过期，刻意不落盘；歌单/收藏走 `LibraryRepository`（playlists.json）持久化；
5. **首页只取 Top50**，不是完整榜单。
...
真实 URL 由 `ResolveDataSpecResolver` 在每次读流时实时解析——WYY 直链 20 分钟过期，预取后长期持有必然失效。
